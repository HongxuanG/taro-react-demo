...
export const formatDateTime = (val, fmt = "YYYY-MM-DD HH:mm:ss") => {
    if (val) {
        ...
        fmt = fmt.replace('YYYY', String(Y)).replace('MM', M.toString().padStart(2, '0')).replace('DD', d.toString().padStart(2, '0')).replace('HH', h.toString().padStart(2, '0')).replace('mm', m.toString().padStart(2, '0')).replace('ss', s.toString().padStart(2, '0'))
